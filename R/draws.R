...
#'
#' @param fit A bayesian fit. Alternatively, you can specify `draws`.
#' @param draws Data frame of draws, as generated by [get_draws()]. Overrides `fit`.
...
#'     attach_draw(draws = draws, index = i)
#' }
attach_draw <- function(fit, draws = get_draws(fit), index, chain, iter, envir = parent.frame()) {
    if (!missing(index)) {
